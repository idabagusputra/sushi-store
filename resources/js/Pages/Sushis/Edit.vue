...
const updateData = (sushi) => {
    form.type = type.value ? type.value.id : sushi.sushi_type_id;
    form.master = master.value ? master.value.id : sushi.sushi_master_id;
    form.name = sushi.name;
    form.quantity = sushi.quantity;
    form.price = sushi.price;
    form.put(route("sushis.update", sushi.id), {
        errorBag: "updateData",
        preserveScroll: true,
        onError: () => {
            if (form.errors.type) {
...
<template>
    <AppLayout title="Sushis">
        ...
        <div class="max-w-7xl mx-auto py-10 sm:px-6 lg:px-8">
            <FormSection @submitted="updateData(sushi)">
                <template #title>Edit Sushi</template>

                <template #description> Edit a sushi data. </template>
                    ...
                    <div class="col-span-6 sm:col-span-4">
                        ...
                        <div class="dropdown relative mt-2 w-full">
                            <button
                                ...
                                aria-expanded="false"
                            >
                                {{ type ? type.name : sushi.sushi_type_name }}
